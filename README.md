...
| `packs/j1/math.js` | 中1 数学 36型（正負の数・文字と式・一次方程式・比例反比例・図形・データ）＋ワンポイント |
| `packs/j1/calc.js` | 中1 計算1枚（60回。math の生成器を使う） |
...
- **答えの種類** `ans.k`: `n`（0以上の整数＋単位）/ `int`（負もOK、「−」ボタン）/ `dec`（小数）/ `frac`（整数＋分子/分母、約分して比較）/ `text`（文字。全半角・大小・空白を無視）/ `expr`（文字式。`sameExpr`＝形をそろえて文字に数を入れ4回一致で正解。×が残る・かっこが残る・項の数が違う＝不正解。`pre:'y='` で接頭辞、記号キーつき）/ `int` に `pre:'x='` で方程式の解 / `pair` / `pick` / `choice` / `self`。
